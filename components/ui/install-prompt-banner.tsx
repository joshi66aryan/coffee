...
export function InstallPromptBanner() {
  ...
  if ((!canInstall && !needsManualInstall) || dismissed) return null

  return (
    <div className="flex items-center gap-3.5 border-b border-brand-300 bg-brand-50 px-4 py-3 sm:px-6">
      {needsManualInstall ? (
        <Share className="h-4.5 w-4.5 shrink-0 text-brand-600" />
      ) : (
      ...
      {/* No Install button on iOS — there is nothing for it to call. */}
      {canInstall && (
